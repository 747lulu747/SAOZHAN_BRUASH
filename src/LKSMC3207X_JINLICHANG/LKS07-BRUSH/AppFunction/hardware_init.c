...
void ADC_init(void)
{
    ...
    ADC_CHN_GAIN_CFG(ADC0, CHN0, ADC_CHANNEL_7, ADC_GAIN3V6);
...
void GPIO_init(void)
{
  ...
  GPIO_PinAFConfig(GPIO0, GPIO_PinSource_3,  AF8_TIMER23); //复用为timer2的编码器T1信号
  GPIO_PinAFConfig(GPIO0, GPIO_PinSource_4,  AF8_TIMER23); //复用为timer2的编码器T2信号
}
...
void UTimer_init(void)
{
    ...
    TIM_QEPStructInit(&TIM_QEPInitStruct);                   /* Timer结构体初始化*/
	  ...
	  TIM_QEPInitStruct.QEP_Mode = QEP_Mode_T1_T2;             /* 编码器模式选择*/
	  ...
	  // TIM_QEPInitStruct.QEP_TH = 65535;                     /* 编码器计数周期*/
		TIM_QEPInitStruct.QEP_TH = 4096;                         /* 编码器计数周期*/
...
void MCPWM_init(void)
{
    ...
    MCPWM_InitStructure.FAIL0_INPUT_EN   = DISABLE ;//FAIL_0CAP
	  ...
	  /* MCPWM0_CNT1 */
		MCPWM_InitStructure.BASE_CNT1_EN = ENABLE;               
    ...
    MCPWM_InitStructure.CH4P_default_output = LOW_LEVEL;    
    MCPWM_InitStructure.CH4N_default_output = LOW_LEVEL;    
