...
int32 vibration_duty_feaback_energy_sum;
int32 vibration_duty_feaback_energy;
...
int32 swing_theta_speed_ref_lpf;
int16 swing_theta_uq_ref;
int32 swing_theta_uq_ref_lpf;
...
int8  vibration_high_freq_flag;
int32 vibration_q_freq_set;
int16 vibration_q_amplite_set;
int16 vibration_q_out;
int16 vibration_q_out_limit;
int8  vibration_q_com_valid;
int16 vibration_q_com_set;
...
PIController swing_pos_pi = {
  ...
  _Q12(0.8),              // 0.2 for high swing freq, and 0.5 for low swing freq
  _Q15(0.0), 
...
void foc_init(void) {
  ...
  swing_theta = 0;
  swing_freq_set = 2.85;
  swing_theta_amplite_set = (int32)(0.4 * Q_CORDIC_2PI / 6.0);
  vibration_high_freq_flag = 0;
  ...
  vibration_q_com_set = _Q15(0.30/6.0);
...
void foc_loop_isr(void) {
  ...
  // 2) theta est
  {
    ...
    // Take about 3 us
    // pll(lh1_raw_lpf_hr>>18, lh0_raw_lpf_hr>>18 ,&lhp_swing_pll_pi, 0, &lh_m_theta, &lh_m_speed_est);
...

  {
    ...
    triangle_theta_normal = triangle_theta = triangle_gen();
    ...
    triangle_theta_notch_fliter_2 = notch_filter_550(triangle_theta);
    ...
    bandpass_out = butter_bandpass_550(triangle_theta);
    return;
    #endif
  }
  ...
  // swing theta pi controller output
  {
    ...
    bt = lh_e_theta - SWING_THETA_BASE;
    // if (u32_abs(bt) < _Q15(0.99)) {
    //   if (!vibration_high_freq_flag) {
    //     vibration_duty_feaback = butter_bandpass_220(bt);
    //     lh_e_theta_nf = notch_filter_220(bt) + SWING_THETA_BASE;
    //   }
    //   else {
    //     vibration_duty_feaback = butter_bandpass_550(bt);
    //     lh_e_theta_nf = notch_filter_550(bt) + SWING_THETA_BASE;
    //   }
    // }
    // else {
    //   lh_e_theta_nf = lh_e_theta;
    // }
    ...
    // LPFFunction_HR_2(_Q15(0.01), lh_e_theta_nf - SWING_THETA_BASE, &lh_e_theta_lpf);
    // lh_e_theta_avg = (lh_e_theta_lpf>>15) + SWING_THETA_BASE;
    ...
    LPFFunction_HR_2(_Q15(0.1), pi_controller(&swing_pos_pi, (int16)swing_theta_err), &swing_theta_speed_ref_lpf);
    swing_theta_speed_ref = swing_theta_speed_ref_lpf >> 15;
    ...
    // LPFFunction_HR_2(_Q15(0.1), lh_m_speed_est<<1, &lh_e_speed_est);
    // lh_e_speed_sum += lh_e_speed_est>>13;
    ...
    //LIMIT(lh_e_speed_err, -_Q15(0.1), _Q15(0.1));
    swing_theta_uq_ref = pi_controller(&swing_speed_pi, lh_e_speed_err);
    // LPFFunction_HR_2(_Q15(0.1), pi_controller(&swing_speed_pi, lh_e_speed_err), &swing_theta_uq_ref_lpf);
    // swing_theta_uq_ref = swing_theta_uq_ref_lpf>>15;
    ...
    // swing out rang check
    {
      if (!swing_out_rang) {
        ...
        if (Q_CORDIC_210_DEG - SWING_THETA_RESTORE_ISO_STRIP < lh_m_theta && lh_m_theta < Q_CORDIC_210_DEG + SWING_THETA_RESTORE_ISO_STRIP) {
    ...
    #define CMP_THETA         (lh_e_theta_avg)
    vibration_q_com_valid = 1;
  ...
  {
    if (vibration_duty_feaback > 0) 
      vibration_duty_feaback_energy_sum += vibration_duty_feaback;
    else
      vibration_duty_feaback_energy_sum -= vibration_duty_feaback;
    vibration_duty_feaback_energy = vibration_duty_feaback_energy_sum>>8;
    vibration_duty_feaback_energy_sum -= vibration_duty_feaback_energy;


    if (!swing_theta_err_too_large_flag) {
      vibration_q_out_fb_cmp = pi_controller(&vibration_pi, vibration_q_com_set - vibration_duty_feaback_energy);
    ...
    if (!vibration_q_com_valid) {
      vibration_pi.integral = 0;
      vibration_q_out_fb_cmp = 0;
    }

    if ((u16_abs(swing_theta_uq_ref) + u16_abs(vibration_q_out)) > _Q15(0.99)) {
      vibration_q_out_limit = _Q15(0.99) - u16_abs(swing_theta_uq_ref);
    }
    else {
      vibration_q_out_limit = _Q15(0.6);
    }
    ...
    if (vibration_q_time_cnt > vibration_q_time) {
      ...
      if (vibration_q_dir) {
        ...
        // if (vibration_q_out < -vibration_q_out_limit)
        //   vibration_q_out = -vibration_q_out_limit;
      }
      else {
        vibration_q_dir = 1;
        vibration_q_out = vibration_q_amplite_set + vibration_q_out_fb_cmp;
  ...
  // Current controller
  {
    ...
    if (swing_out_rang) {
      ...
      temp = (int32)swing_theta_uq_ref + (int32)vibration_q_out;
...
void foc_vibration_set(float swing_fre, int32 swing_duty, int32 vibration_fre, int32 vibration_duty) {
  ...
  if (vibration_fre > 250)
    vibration_high_freq_flag = 1;
  else
    vibration_high_freq_flag = 0;

  swing_theta_amplite_set = swing_duty;
  swing_theta_step = (int32)(SWING_CNT * 4.0 * swing_fre * swing_theta_amplite_set / PWM_CYCLE_Freq);
  swing_theta_max = SWING_THETA_BASE + swing_theta_amplite_set;
  swing_theta_min = SWING_THETA_BASE - swing_theta_amplite_set;

  swing_lh_lpf_cofe = _Q15(500.0 * swing_fre * CORDIC_2PI * Ts);
  lhp_swing_pll_pi.Kp = (int16)(1.0 * 0.5 * swing_fre * CORDIC_2PI);
  lhp_swing_pll_pi.Ki = _Q15(0.25 * swing_fre * swing_fre * CORDIC_2PI * CORDIC_2PI * Ts);

  // q vibration
  vibration_q_freq_set = vibration_fre;
  vibration_q_amplite_set = vibration_duty;
  if (!vibration_high_freq_flag)
    vibration_q_com_set = vibration_duty / 6;
  else
    vibration_q_com_set = vibration_duty / 55;
  vibration_q_out = vibration_q_amplite_set;
  vibration_q_time = (PWM_CYCLE_Freq / vibration_q_freq_set / 2);
  vibration_q_step = vibration_q_freq_set * Q_CORDIC_2PI / PWM_CYCLE_Freq;
