...
#define VIBRATION_DUTY_5            (_Q15(0.50))

extern int16 VPS_ADC;
extern int8 work_flag;
void swing_vibration_set(void) {
  static int32 cnt = 0;
  static int8 last_level = 1;
  static int8 set_level = 1;
  int8 cur_level;

  if (VPS_ADC < 1200) {
    cur_level = 0;
  }
  else if (VPS_ADC < 1400) {
    cur_level = 1;
  }
  else if (VPS_ADC < 1600) {
    cur_level = 2;
  }
  else if (VPS_ADC < 1800) {
    cur_level = 3;
  }
  else if (VPS_ADC < 2000) {
    cur_level = 4;
  }
  else if (VPS_ADC < 2200) {
    cur_level = 5;
  }
