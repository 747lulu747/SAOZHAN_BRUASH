...
void foc_core_isr_loop(void) {
  ...
  // 1) reconstruct current a b c
  {
    ...
    theta_speed.lh0_raw = ((ADC0->CH[8].DATA & 0xFFF)<<3) - lh_adc_offset_0;
    theta_speed.lh1_raw = ((ADC0->CH[7].DATA & 0xFFF)<<3) - lh_adc_offset_1;
  ...
  // 2) theta est
  {
    ...
    theta_speed.lh_e_theta = theta_speed.lh_m_theta;
...
int foc_core_set_sv(float swing_fre, int32 swing_duty, int32 vibration_fre, int16 vibration_duty) {
  ...
  if (vibration_fre > 350) {
    ...
    swing_speed_pi.Kp = _Q8_W16(10.0);
    swing_speed_pi.Ki = _Q15(0.05);
...
void pos_speed_loop(void) {
  ...
  // swing.theta_uq_ref = pi_controller(&swing_pos_pi, (int16)swing.theta_err);
  // return;
