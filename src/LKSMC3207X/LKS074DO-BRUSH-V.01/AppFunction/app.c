...
void app_hardware_init(void) {
  ...
  GPIO_InitStruct.GPIO_Pin = DCDC_EN_PIN;
  GPIO_Init(DCDC_EN_PORT, &GPIO_InitStruct);
  GPIO_SetBits(DCDC_EN_PORT, DCDC_EN_PIN);

  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStruct.GPIO_Pin = GEARS_LED_1_PIN | GEARS_LED_2_PIN | GEARS_LED_3_PIN;
  GPIO_Init(GEARS_LED_1_PORT, &GPIO_InitStruct);
...
void led_blink(void) {
  ...
  static uint32 _1m_tick = 0;
  static uint8 led_sw = 0;
