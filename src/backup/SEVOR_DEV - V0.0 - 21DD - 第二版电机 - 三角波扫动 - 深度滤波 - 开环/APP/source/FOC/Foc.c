...
void foc_init(void) {
  ...
  swing_lh_lpf_cofe = _Q15(1.0 * swing_freq_set * CORDIC_2PI * Ts);
  swing_theta_err_too_large_cnt = 0;
  swing_theta_err_too_large_flag = 0;
  swing_theta_d_set = MIN_D_DUTY;
  lhp_swing_pll_pi.Kp = 2.0 * 5.0 * swing_freq_set * CORDIC_2PI;
  lhp_swing_pll_pi.Ki = _Q12(25.0 * swing_freq_set * swing_freq_set * (CORDIC_2PI_SQ) * Ts);
...
void foc_loop_isr(void) {
  ...
  // swing theta output
  {
    ...
    // Q output
    {
      ...
      // if (swing_theta_err > 2*Q_CORDIC_1_DEG)
      //   swing_theta_err = 2*Q_CORDIC_1_DEG;
      // else if (swing_theta_err < -2*Q_CORDIC_1_DEG)
      //   swing_theta_err = -2*Q_CORDIC_1_DEG;
