...
void foc_init(void) {
  ...
  lhp_swing_pll_pi.Kp = (int16)(2.0 * 1.0 * swing_freq_set * CORDIC_2PI);
  lhp_swing_pll_pi.Ki = _Q15(1.0 * swing_freq_set * swing_freq_set * CORDIC_2PI * CORDIC_2PI * Ts);
  ...
  vibration_q_amplite_set = _Q15(0.3);
...
void foc_loop_isr(void) {
  ...
  // 2) theta est
  {
    ...
    lh_e_speed_sum += (lh_e_speed_est>>7);
  ...
  // swing theta pi controller output
  {
    ...
    swing_theta_uq_ref = pi_controller(&swing_speed_pi, swing_theta_speed_ref - lh_e_speed_fb);
