...
* All Rights Reserved
**************************************************************************************************/
#ifndef __TAILWINDDETECT_H_
#define __TAILWINDDETECT_H_
...
typedef struct
{
    ...
    uint8  AntiFlag;                                        // 不同转速的反转标志
	  uint8  ThetaStep;																				// 电机角度状态标志
	  uint16 CCWtoCWWaitTime;																	// PLL正转到反转等待时间
//	uint8  HighTailWindSpeedResure;													// 高速逆风确认
    uint16 TailWindOverTime;
...
#define BaseTimerFre                    (16000)          //Hz
#define ThailWindSpeedBase              ((BaseTimerFre/2)*60/Pole_Pairs)//没必要除以2
//#define ThailWindSpeedBase              ((BaseTimerFre)*60/Pole_Pairs)//没必要除以2
#define TailWindStartMinSpeed           (uint16)(BaseTimerFre/TailWind_Time*60.0/Pole_Pairs)      //顺风启动的最低转速



extern MotorTailWindTypeDef   TailWindDetect;
