...
int16 anti_cogging_compesation_iq_lpf;
...
#define SWING_PI_LIMI              (0.75)
#define SWING_PI_INTERGL_LIMIT     (0.75)
...
void foc_init(void) {
  ...
  lhp_swing_pll_pi.Kp = (int16)(2.0 * 5.0 * swing_freq_set * CORDIC_2PI);
  lhp_swing_pll_pi.Ki = _Q12(25.0 * swing_freq_set * swing_freq_set * CORDIC_2PI * CORDIC_2PI * Ts);
  if (lhp_swing_pll_pi.Kp < 62) lhp_swing_pll_pi.Kp = 62;
  if (lhp_swing_pll_pi.Ki < 183) lhp_swing_pll_pi.Ki = 183;
  ...
  if (lhp_swing_pll_deep_pi.Kp < 62) lhp_swing_pll_pi.Kp = 62;
  if (lhp_swing_pll_deep_pi.Ki < 183) lhp_swing_pll_pi.Ki = 183;
  ...
  if (lhp_vibration_pll_pi.Kp < 62) lhp_swing_pll_pi.Kp = 62;
  if (lhp_vibration_pll_pi.Ki < 183) lhp_swing_pll_pi.Ki = 183;
...
void foc_loop_isr(void) {
  ...
  // 2) theta est
  {
    ...
    // pll(lh1_raw_lpf_hr>>15, lh0_raw_lpf_hr>>15 ,&lhp_swing_pll_pi, (5), &lh_m_theta, &lh_m_speed_est);
    ...
    // LPFFunction_HR_2(lh_e_speed_lpf_cofe, lh_m_speed_est<<1, &lh_e_speed_est);
    LPFFunction_HR_2(_Q15(0.005), lh_m_speed_est<<1, &lh_e_speed_est);
  ...
  {
    static uint8 use_anti_cogging = 0;
    ...
    // iq_ref = pi_controller(&swing_pos_pi, (int16)swing_theta_err);
    // iq_ref += anti_cogging_compesation_iq<<2;
    // iq_ref -= vibration_q_out>>2;

    // swing_theta_err = theta_distance(swing_theta_out, lh_e_theta_deep);
    
    // VALUE_LIMIT(swing_theta_err, _Q15(0.1), _Q15(-0.1));
    // iq_ref = ser_anti_intergral_pi_controller(&swing_pos_pi, (int16)swing_theta_err);
    // iq_ref = anti_integral_windup_pi_controller(&swing_pos_pi, (int16)swing_theta_err);
    ...
    #if (!ANTI_COGGING_CALIBRATION)
    anti_cogging_idx = anti_cogging_compesation_theta_to_idx_round(lh_m_theta);
    // anti_cogging_compesation_iq = anti_cogging_compesation_data[anti_cogging_idx];
    anti_cogging_compesation_iq = LPFFunction(_Q15(0.01), anti_cogging_compesation_data[anti_cogging_idx],  anti_cogging_compesation_iq);
    // if (use_anti_cogging)
    //   iq_ref = pi_controller(&swing_speed_pi, (int32)2000 - lh_e_speed_fb) + anti_cogging_compesation_iq;
    // else
    //   iq_ref = pi_controller(&swing_speed_pi, (int32)2000 - lh_e_speed_fb);
    #endif

    swing_theta_err = theta_distance(swing_theta_out, lh_e_theta);
    swing_theta_speed_ref = pi_controller(&swing_pos_pi, (int16)swing_theta_err);
    if (use_anti_cogging)
      iq_ref = pi_controller(&swing_speed_pi, (int32)swing_theta_speed_ref - lh_e_speed_fb) + anti_cogging_compesation_iq;
    else
      iq_ref = pi_controller(&swing_speed_pi, (int32)swing_theta_speed_ref - lh_e_speed_fb);
    // swing_theta_uq_ref = pi_controller(&swing_speed_pi, (int32)2000 - lh_e_speed_fb);
    ...
    //   swing_theta_ip_out = pi_controller(&swing_pos_pi, swing_theta_err) + swing_theta_competation_out;
    // }
    // else {
    //   swing_theta_ip_out = pi_controller(&swing_pos_pi, swing_theta_err);
    //   swing_compesation_feedforward_calibration();
    // }
    
    // swing_theta_ip_out = pi_controller(&swing_pos_pi, swing_theta_err>>3);
    // id_ref = _Q15(0.2);
    // iq_ref = swing_theta_ip_out;
    ...
    if (swing_open_up_compesation_cnt > 10) {
      ...
      // swing_pos_pi.integral += (swing_freq_set_q15 * _Q15(0.01)) >> 15;
      // swing_pos_pi.integral = 0;
      // swing_pos_pi.integral = swing_theta_ip_out << 8;
      // swing_pos_pi.integral = swing_theta_open_compesation;
      // t = swing_theta_out>>1;
      // THETA_LIMIT(t, Q_CORDIC_2PI);
      // lh_m_theta = t;
      // lh_e_speed_lpf_cofe = _Q15(0.01);
    }
    ...
    // iq_ref += swing_theta_open_compesation;

    // if (swing_theta_out > swing_theta_max - OPEN_CTRL_THETA) {
    //   swing_theta_ip_out += (swing_freq_set_q15 * _Q15(0.01)) >> 15;
    //   // swing_pos_pi.integral += (swing_theta_max - OPEN_CTRL_THETA) - swing_theta_out;
    // }
    ...
    //   // swing_pos_pi.integral = swing_theta_min + OPEN_CTRL_THETA - swing_theta_out;;
    // }
    // else {
    //   swing_theta_ip_out = pi_controller(&swing_pos_pi, swing_theta_err);
    // }
    ...
    // swing_theta_ip_out = anti_integral_windup_pi_controller(&swing_pos_pi, swing_theta_err);
    // if (swing_pi_clear) {
    //   // swing_pi_clear = 0;
    ...
    //   }
    //   else {
    //     swing_pos_pi.integral = swing_theta_ip_out = -(swing_freq_set_q15 * _Q15(0.01)) >> 15;
  ...
  {
    {
        ...
        // static int32 pct_cnt = 0;
        // pct_cnt++;
        // if (pct_cnt > 0) {
        //   pct_cnt = 0;
        //   pos_close_test_theta += 1;
        //   THETA_LIMIT(pos_close_test_theta, Q_CORDIC_2PI);
        // }
        ...
        // pct_Is = ser_anti_intergral_pi_controller(&pos_close_test_pi, (int16)pos_err);
        // pct_Is = pi_controller(&pos_close_test_pi, (int16)pos_err);
        // pct_Is = ser_pi_controller(&pos_close_test_pi, (int16)pos_err);
        if (0 == anti_cogging_compesation_flag) {
          iq_ref = pi_controller(&pos_close_test_pi, (int16)pos_err);
          //if (Q_CORDIC_210_DEG < lh_m_theta && lh_m_theta < Q_CORDIC_270_DEG) {
            // anti_cogging_compesation(working_theta, anti_cogging_compesation_dir, Iq);
          //}
          // mtpa_out(pct_Is, &id_ref, &iq_ref);
          // if (u32_abs(pos_err) < Q_CORDIC_3_DEG)
          anti_cogging_compesation(lh_m_theta, anti_cogging_compesation_dir, iq_ref);
        }
        else {
          iq_ref = pi_controller(&pos_close_test_pi, (int16)pos_err);
          // iq_ref = 0;
          ...
          anti_cogging_compesation_iq = anti_cogging_compesation_data[anti_cogging_idx];
          iq_ref += anti_cogging_compesation_iq;
          // pct_Is = anti_cogging_compesation_iq;
        }
    ...
    if (POS_E == ctrl_mode) {
      ...
      // err deal with
      {
        ...
        if (swing_out_rang) {
          ...
          // Uq = _Q15(0.3);
          // Uq = swing_theta_uq_ref + vibration_q_out;
          // Uq = 0;
        }
      }
    #endif
...
void anti_cogging_compesation(int32 theta, int8 dir, int16 anti_iq) {
  ...
  stc_index = anti_cogging_compesation_theta_to_idx_round(theta);
  // if (dir < 0)
  //   stc_index += (ANTI_COGGING_COMPESATIN_SIZE/2);
  // anti_cogging_compesation_data[stc_index] = anti_iq;
  anti_cogging_compesation_iq = LPFFunction(_Q15(0.01), anti_iq, anti_cogging_compesation_iq);
