...
int16 lh_m_speed_vibration_est;
...
void foc_init(void) {
  ...
  swing_theta_step = (int32)(4 * swing_freq_set * swing_theta_amplite_set / PWM_CYCLE_Freq);
  swing_lh_lpf_cofe = _Q15(Ts * swing_freq_set * CORDIC_2PI);

  lhp_swing_pll_pi.Kp = 2.0 * 2.0 * swing_freq_set * CORDIC_2PI * 1.0;
  lhp_swing_pll_pi.Ki = _Q12(4.0 * swing_freq_set * swing_freq_set * (CORDIC_2PI_SQ) * Ts);
  ...
  vibration_q_amplite_set = _Q15(0.25);
...
void foc_loop_isr(void) {
  ...
  // 1) reconstruct current a b c
  {
    ...
    // lh0_raw = ((ADC0->CH[1].DATA & 0xFFF)<<3) - 13894;
    // lh1_raw = ((ADC0->CH[2].DATA & 0xFFF)<<3) - 13337;
  ...
  // swing theta output
  {
    ...
    swing_theta_err = swing_theta_out_45_deg_delay - lh_e_theta;

    // if (swing_theta_forward_compesation_flag) {
    //   swing_compesation_feedforward_calibration();
    //   swing_theta_compesation = pi_controller(&swing_pi, swing_theta_err) + swing_theta_competation_out;
    // }
    // else {
    //   swing_theta_compesation = pi_controller(&swing_pi, swing_theta_err);
    //   swing_compesation_feedforward_calibration();
    // }
    swing_theta_compesation = pi_controller(&swing_pi, swing_theta_err);
  ...
  // vibration compensation and output
  {
    ...
    if (vibration_q_time_cnt > vibration_q_time) {
      vibration_q_time_cnt = 0;
      vibration_q_max = _Q15(0.99) - swing_theta_compesation;
      if (vibration_q_dir) {
        vibration_q_dir = 0;
  ...
  // swing theta valid rang dealwith
  {
    ...
    // }
    // else {
    //   if (Q_CORDIC_30_DEG < lh_m_theta && lh_m_theta < Q_CORDIC_90_DEG) {
  ...
  // Current controller
  {
    if (vibration_out_rang) {
      ...
      Uq = vibration_q_out + swing_theta_compesation;
...
void swing_compesation_feedforward_calibration(void) {
  ...
  if (0 <= stc_cnt && stc_cnt < (uint32)(3 * PWM_CYCLE_Freq)) {
    ...
    swing_feedforward[stc_index] = swing_theta_compesation;
    stc_cnt++;
  }
  else {
    if (stc_cnt > 0) {
