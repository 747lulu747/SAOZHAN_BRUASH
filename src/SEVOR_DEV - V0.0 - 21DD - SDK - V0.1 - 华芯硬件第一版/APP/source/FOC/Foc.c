...
/************************************************************/
// Local variable
sw_vb_state_e sw_vb_state, sw_vb_state_last, sw_vb_state_req;
int32 com_theta_sum;
int32 com_theta_avg;
int32 com_cnt;
int16 com_speed_ref;
...
PIController lh_theta_pll_pi = {
  1,                      // position mode
  (int16)(1.0 * 0.5 * SWING_FREQ_DEFAULT * CORDIC_2PI), 
  _Q15(0.25 * SWING_FREQ_DEFAULT * SWING_FREQ_DEFAULT * CORDIC_2PI * CORDIC_2PI * Ts), 
  0,                      // integral
...
PIController swing_pos_pi = {
  1,                              // position mode
  _Q12(0.4),                      // Kp, 0.2 for high swing freq, and 0.5 for low swing freq
  _Q15(0.0),                      // Ki
  0,                              // integral
...
PIController pb_pos_pi = {
  1,                              // position mode
  _Q12(1.0),                      // Kp, 0.2 for high swing freq, and 0.5 for low swing freq
  _Q15(0.0),                      // Ki
  0,                              // integral
...
// int16 pos_speed_loop(int32 input theta, int32 target_theta, 
//                     PIController *pos_pi, 
//                     PIController *sp_pi, 
//                     PIController *pll_pi);
...
void foc_init(void) {
  ...
  m_theta_align.theta_m = 41255;
  m_theta_align.theta_e = 82510;
  
  // swing.theta_m_base = (SWING_THETA_E_BASE<<1);
  // THETA_LIMIT(swing.theta_m_base, Q_CORDIC_2PI);
  ...
  swing.theta_step = (int32)(SWING_CNT * 4.0 * swing.freq_set * swing.amplitude / PWM_CYCLE_Freq);
  ...
  swing.out_rang_flag = 0;
  __working_theta = 0;
  sw_vb_state_last = sw_vb_state = sw_vb_state_req = SW_VB_INIT;
...
int foc_core_open_drive(int16 duty, int32 theta) {
  sw_vb_state_req = SW_VB_OPEN_DRIVE;
...
int foc_core_m_theta_align_exit(void) {
  if (SW_VB_ALIGN != sw_vb_state)
    return (int)ERR_E_THETA_ALIGN_NOT_IN_ALIGN_STATE;

  sw_vb_state_req = sw_vb_state_last;
...
int32 foc_core_get_align_m_theta(void) {
  return m_theta_align.theta_m;
}

int foc_core_pullback(void) {
  if (SW_VB_PB != sw_vb_state) {
    sw_vb_state_req = SW_VB_PB;
    pb.state = PB_INIT;
  }
  return ERR_OK;
}
...
int foc_core_set_sw_base(void) {
  sw_vb_state_req = SW_VB_SET_BASE;
  return ERR_OK;
}
...
void foc_loop_isr(void) {
  ...
  {
    abc_cur.Ib = mcCurOffset_v - ((ADC0->CH[1].DATA & 0xFFF)<<3);
    abc_cur.Ic = mcCurOffset_w - ((ADC0->CH[2].DATA & 0xFFF)<<3);
    abc_cur.Ia = 0 - abc_cur.Ib -abc_cur.Ic;

    theta_speed.lh0_raw = ((ADC0->CH[7].DATA & 0xFFF)<<3) - 15604;
    theta_speed.lh1_raw = ((ADC0->CH[6].DATA & 0xFFF)<<3) - 15552 - 100;
  ...
  if (sw_vb_state_req != sw_vb_state) {
    sw_vb_state_last = sw_vb_state;
    sw_vb_state = sw_vb_state_req;
  }
...
void pos_speed_loop(void) {
  ...
  // theta_speed.lh_e_theta_sum += theta_speed.lh_e_theta;
  DIV_SDiv(theta_speed.lh_e_theta_sum, 100);
  ...
  LPFFunction_HR_2(_Q15(0.1), pi_controller(&swing_pos_pi, (int16)swing.theta_err), &swing.theta_speed_ref_lpf);
...
//   // calc the avg theta
//   pos_speed_ctrl.theta_sum += theta;
//   DIV_SDiv(pos_speed_ctrl.theta_sum, 100);
//   while(DIV_Div_IsBusy());
//   DIV_SDiv_Result(&div_quotient, &div_remainder);
//   pos_speed_ctrl.theta_avg = div_quotient;
//   pos_speed_ctrl.theta_sum -= pos_speed_ctrl.theta_avg;

//   // pos pi output
//   theta_err = theta_distance(target_theta, pos_speed_ctrl.theta_avg);
//   LPFFunction_HR_2(_Q15(0.1), pi_controller(pll_pi, (int16)theta_err), &com_speed_ref_hrlpf);
//   com_speed_ref = com_speed_ref_hrlpf >> 15;
  
//   // pll output speed feadback
//   pos_speed_ctrl.speed_theta += pos_speed_ctrl.pi_speed;
//   pos_speed_ctrl.speed_fb = pi_controller(sp_pi, (pos_speed_ctrl.theta_avg - pos_speed_ctrl.speed_theta)) << 5;
  
//   // speed pi output
//   pos_speed_ctrl.speed_err = com_speed_ref - pos_speed_ctrl.speed_fb;
//   return pi_controller(sp_pi, pos_speed_ctrl.speed_err);
// }
...
void foc_vibration_set(float swing_fre, int32 swing_duty, int32 vibration_fre, int16 vibration_duty) {
  ...
  if (vibration_fre > 250) {
    swing_speed_pi.Kp = _Q10(8.0);
    vibration.high_freq_flag = 1;
  }
  else {
    swing_speed_pi.Kp = _Q10(10.0);
    vibration.high_freq_flag = 0;
  }
